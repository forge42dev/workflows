...
json_to_assoc_array () {
  ...
  local -n assoc_array_ref=$1
  local json="${2}"
  local assoc_array_def=$(echo "$json" | jq -r '. | paths as $p | [($p|map(tostring)|join("_")), getpath($p)] | { (.[0]): .[1] }' | jq -rs "add | to_entries | map(\"${assoc_array_name}[\\\"\(.key)\\\"]=\\\"\(.value)\\\";\") | .[]")
  debug "assoc_array_def=$assoc_array_def"
